...
TEST(map, SwapMap) {
  ...
  s21::map<int, int> my_map_2;
  my_map_2 = std::move(my_map);
  auto z = (*(my_map_2.begin()));
  EXPECT_EQ(z, 3);
}
...
TEST(map, MergeMap) {
  ...
  EXPECT_TRUE((*my_map.begin() + 1) == 2);
  EXPECT_TRUE((*my_map.begin() + 2) == 3);
  EXPECT_TRUE((*my_map.begin() + 3) == 4);
}
